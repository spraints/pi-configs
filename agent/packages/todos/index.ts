...
const TodoParams = Type.Object({
  ...
  body: Type.Optional(Type.String({ description: "Markdown body. Update replaces; append adds." })),
...
function normalizeTodoSettings(raw: Partial<TodoSettings>): TodoSettings {
  ...
  const gcDays = Number.isFinite(raw.gcDays)
    ? (raw.gcDays ?? DEFAULT_TODO_SETTINGS.gcDays)
...
export default function todosExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    description:
      ...
      "Use claim/release for ownership. IDs accept TODO-<hex> or the raw hex filename.",
    promptSnippet: `Manage file-based todos in ${todosDirLabel}. Claim before working; release or close when done.`,
    ...
    renderResult(result, { expanded, isPartial }, theme) {
      ...
      const detailsWithTodo = details as Extract<typeof details, { todo: TodoRecord }>;
      if (!detailsWithTodo.todo) {
        const text = result.content[0];
        return new Text(text?.type === "text" ? text.text : "", 0, 0);
      }

      let text = renderTodoDetail(theme, detailsWithTodo.todo, expanded);
      const actionLabel =
